...
function battleplan_getHuzzaz( $atts, $content = null ) {
	...
	$buildHuzzaz .= '$.post({ url : "/wp-admin/admin-ajax.php", data : { action: "load_cue_mp3", id: postID, timezone: timezone, userValid: userValid, userLoc: userLoc, pagesViewed: pageViews, uniqueID: uniqueID }, success: function( response ) { console.log(response); } });';
	$buildHuzzaz .= '}, 10000)';
	$buildHuzzaz .= '</script>';
...
function battleplan_load_cue_mp3_ajax() {
	...
	echo do_shortcode('[get-huzzaz id="'.$id.'" vpp="'.$vpp.'" height="'.$height.'" bg="'.$bg.'" color="'.$color.'" button="'.$button.'" highlight="'.$highlight.'"]');
}
