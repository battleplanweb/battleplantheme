<?php
/**
 * Home Base — Web Push (customer-keyed, payload-carrying)
 * ---------------------------------------------------------------------------
 ...
 *
 * Subscriptions are keyed on customer_id (table home_base_push_subscriptions),
 * deduped by endpoint hash. Dispatch: hb_push_send_to_customer() /
 * hb_push_broadcast(). @package battleplan
 */
...
function hb_b64url_encode( string $bin ): string { return rtrim( strtr( base64_encode( $bin ), '+/', '-_' ), '=' ); }
function hb_b64url_decode( string $txt ): string {
	$txt = strtr( $txt, '-_', '+/' );
	$pad = strlen( $txt ) % 4;
 ...
 */
function hb_vapid_keys(): array {
	$pub  = get_option( 'home_base_vapid_public', '' );
	$priv = get_option( 'home_base_vapid_private', '' );
	...
	$pub  = hb_b64url_encode( $raw_pub );
	update_option( 'home_base_vapid_public', $pub, false );
	update_option( 'home_base_vapid_private', $pem, false );
	return [ 'public' => $pub, 'private' => $pem ];
}

/** The public application server key (b64url) the browser subscribes with. */
function hb_vapid_public(): string {
	$k = hb_vapid_keys();
	return $k['public'] ?? '';
}

/** True when push can actually be sent (keys + crypto present). */
function hb_push_ready(): bool {
	return hb_vapid_public() !== '' && function_exists( 'hash_hkdf' ) && function_exists( 'openssl_encrypt' );
}
...
function hb_der_to_raw( string $der ): string {
	$off = 0;
...
function hb_vapid_jwt( string $audience ): string {
	$keys = hb_vapid_keys();
	if ( ! $keys ) return '';
	$sub = (string) ( function_exists( 'hb_get' ) ? hb_get( 'push_contact', '' ) : '' );
	if ( $sub === '' ) $sub = 'mailto:' . get_option( 'admin_email' );

	$header  = hb_b64url_encode( wp_json_encode( [ 'typ' => 'JWT', 'alg' => 'ES256' ] ) );
	$payload = hb_b64url_encode( wp_json_encode( [
		'aud' => $audience,
	...
	return $signing . '.' . hb_b64url_encode( hb_der_to_raw( $sig ) );
}
 ...
 */
function hb_ec_pub_from_raw( string $raw ) {
 ...
 */
function hb_encrypt_payload( string $payload, string $ua_public, string $auth ): string {
	if ( ! hb_push_ready() ) return '';
	...
	$ua_key = hb_ec_pub_from_raw( $ua_public );
	if ( ! $ua_key ) return '';
...
function hb_rest_push_subscribe( WP_REST_Request $request ) {
	...
	if ( $endpoint === '' || $p256dh === '' || $auth === '' ) {
		return new WP_Error( 'hb_bad', 'Invalid subscription.', [ 'status' => 400 ] );
	}

	global $wpdb;
	$wpdb->query( $wpdb->prepare(
		"INSERT INTO " . hb_table( 'push_subscriptions' ) . "
...
function hb_rest_push_unsubscribe( WP_REST_Request $request ) {
	$customer = hb_current_customer( $request );
	if ( ! $customer ) return new WP_Error( 'hb_auth', 'Not signed in.', [ 'status' => 401 ] );
	$body = json_decode( $request->get_body(), true ) ?: [];
	...
	if ( $endpoint !== '' ) {
		global $wpdb;
		$wpdb->delete( hb_table( 'push_subscriptions' ), [ 'endpoint_hash' => hash( 'sha256', $endpoint ) ] );
	}
	return rest_ensure_response( [ 'ok' => true ] );
}

/** REST: GET /push/meta — the VAPID public key + whether push is available. */
function hb_rest_push_meta( WP_REST_Request $request ) {
	return rest_ensure_response( [ 'ready' => hb_push_ready(), 'vapidPublic' => hb_vapid_public() ] );
}
 ...
 */
function hb_push_deliver( array $subrow, array $payload ): int {
	if ( ! hb_push_ready() ) return 0;
	$endpoint = $subrow['endpoint'];
	$origin   = (string) wp_parse_url( $endpoint, PHP_URL_SCHEME ) . '://' . (string) wp_parse_url( $endpoint, PHP_URL_HOST );

	$body = hb_encrypt_payload(
		wp_json_encode( $payload ),
		hb_b64url_decode( $subrow['p256dh'] ),
		hb_b64url_decode( $subrow['auth'] )
	);
	if ( $body === '' ) return 0;

	$jwt = hb_vapid_jwt( $origin );
	if ( $jwt === '' ) return 0;

	$res = wp_remote_post( $endpoint, [
		'headers' => [
			'Authorization'    => 'vapid t=' . $jwt . ', k=' . hb_vapid_public(),
	...
	if ( $code === 404 || $code === 410 ) {
		global $wpdb;
		$wpdb->delete( hb_table( 'push_subscriptions' ), [ 'id' => (int) $subrow['id'] ] );
	}
	return $code;
}
 ...
 */
function hb_push_send_to_customer( int $customer_id, array $note, int $sent_by = 0 ): int {
	global $wpdb;
	$note = wp_parse_args( $note, [ 'title' => hb_app_name(), 'body' => '', 'url' => home_url( '/' . HOME_BASE_SLUG . '/' ) ] );

	// Log to the notifications table (also feeds the in-app list) regardless of push success.
	$wpdb->insert( hb_table( 'notifications' ), [
	...
	$subs = $wpdb->get_results( $wpdb->prepare(
		"SELECT * FROM " . hb_table( 'push_subscriptions' ) . " WHERE customer_id = %d", $customer_id
	), ARRAY_A ) ?: [];

	$ok = 0;
	foreach ( $subs as $s ) {
		$code = hb_push_deliver( $s, $note );
 ...
 */
function hb_push_broadcast( array $customer_ids, array $note, int $sent_by = 0 ): array {
	$delivered = 0;
	$ids = array_unique( array_map( 'intval', $customer_ids ) );
	foreach ( $ids as $cid ) {
		if ( $cid > 0 ) $delivered += hb_push_send_to_customer( $cid, $note, $sent_by );
	}
 ...
 * `home_base_push_segment` filter. Built-in: 'all', 'filter_due'.
 */
function hb_push_segment_ids( string $segment ): array {
	global $wpdb;
	if ( $segment === 'all' ) {
		return array_map( 'intval', $wpdb->get_col(
			"SELECT DISTINCT customer_id FROM " . hb_table( 'push_subscriptions' )
		) );
	}
	...
	return array_map( 'intval', (array) apply_filters( 'home_base_push_segment', [], $segment ) );
}
...
add_action( 'rest_api_init', function () {
	$ns = 'home-base/v1';
	register_rest_route( $ns, '/push/meta',        [ 'methods' => 'GET',  'callback' => 'hb_rest_push_meta',        'permission_callback' => '__return_true' ] );
	register_rest_route( $ns, '/push/subscribe',   [ 'methods' => 'POST', 'callback' => 'hb_rest_push_subscribe',   'permission_callback' => '__return_true' ] );
	register_rest_route( $ns, '/push/unsubscribe', [ 'methods' => 'POST', 'callback' => 'hb_rest_push_unsubscribe', 'permission_callback' => '__return_true' ] );

	// In-app notification list + unread badge.
	register_rest_route( $ns, '/notifications', [ 'methods' => 'GET', 'callback' => 'hb_rest_notifications', 'permission_callback' => '__return_true' ] );
	register_rest_route( $ns, '/notifications/read', [ 'methods' => 'POST', 'callback' => 'hb_rest_notifications_read', 'permission_callback' => '__return_true' ] );
} );

/** GET /notifications — the customer's recent notifications + unread count. */
function hb_rest_notifications( WP_REST_Request $request ) {
	$customer = hb_current_customer( $request );
	if ( ! $customer ) return new WP_Error( 'hb_auth', 'Not signed in.', [ 'status' => 401 ] );
	global $wpdb;
	$t   = hb_table( 'notifications' );
...
function hb_rest_notifications_read( WP_REST_Request $request ) {
	$customer = hb_current_customer( $request );
	if ( ! $customer ) return new WP_Error( 'hb_auth', 'Not signed in.', [ 'status' => 401 ] );
	global $wpdb;
	$wpdb->query( $wpdb->prepare(
		"UPDATE " . hb_table( 'notifications' ) . " SET read_at = %s WHERE customer_id = %d AND read_at IS NULL",
