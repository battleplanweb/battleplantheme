...
/*--------------------------------------------------------------
# Customer Feedback Forwarder (→ Site Pulse Emails)
--------------------------------------------------------------*/

/**
 * Generic, per-client forwarder: when a contact-form submission is FLAGGED as a customer-service type,
 * POST it (HMAC-signed) to a Site Pulse install where it lands under Customer Feedback → Emails. A no-op
 * until a site configures it via the bp_feedback_* filters, so it ships dark for every client and each
 * company decides its own flagging/destination. Signing contract matches the survey forwarder:
 *   sig = hash_hmac('sha256', rawBody, secret), sent as Authorization: Bearer <sig>.
 *
 * Filters a site sets (e.g. in its own functions-*.php):
 *   bp_feedback_forward_url        — receiver, e.g. https://rovin.work/wp-json/site-pulse/v1/email
 *   bp_feedback_forward_secret     — shared HMAC secret
 *   bp_feedback_category_field     — form field holding the customer's category pick (default 'user-category')
 *   bp_feedback_forward_categories — array of category VALUES that forward (case-insensitive)
 *   bp_feedback_brand              — ($brand, $ctx) brand label for the row
 *   bp_feedback_payload            — ($payload, $ctx) final tweak hook
 */
...
function bp_feedback_forward_submission($email, $ctx, $sent) {
	...
	if ($endpoint === '' || $secret === '' || empty($forward)) return; // not configured → do nothing
	...
	if (!in_array(strtolower(trim($category)), $forward, true)) return; // not a flagged type
