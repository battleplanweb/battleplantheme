<?php
/**
 * Home Base — Progressive Web App layer
 * ---------------------------------------------------------------------------
 ...
 * Standalone — copied from the Site Pulse PWA pattern, namespaced hb_*, with no
 ...
 *     /home-base-app.webmanifest
 *     /home-base-sw            (extensionless on purpose — nginx on WP Engine
...
if ( ! defined( 'HOME_BASE_PWA_VERSION' ) ) define( 'HOME_BASE_PWA_VERSION', '1.0.0' );

if ( ! defined( 'HOME_BASE_PWA_MANIFEST_FILE' ) ) define( 'HOME_BASE_PWA_MANIFEST_FILE', 'home-base-app.webmanifest' );
if ( ! defined( 'HOME_BASE_PWA_SW_FILE' ) )       define( 'HOME_BASE_PWA_SW_FILE', 'home-base-sw' );
...
/** Resolved PWA config for this site. Filterable via `home_base_pwa_config`. */
function hb_pwa_config(): array {
	$app_name = function_exists( 'hb_app_name' ) ? hb_app_name() : 'Home Base';

	$short = (string) ( function_exists( 'hb_get' ) ? hb_get( 'pwa_short_name', '' ) : '' );
	if ( $short === '' ) {
	...
	$start = home_url( '/' . ( defined( 'HOME_BASE_SLUG' ) ? HOME_BASE_SLUG : 'home-base' ) . '/' );
	...
	$cfg = [
		'name'        => $app_name,
		'short_name'  => $short,
		'theme_color' => (string) ( function_exists( 'hb_get' ) ? hb_get( 'theme_color', '#0f766e' ) : '#0f766e' ),
		'bg_color'    => (string) ( function_exists( 'hb_get' ) ? hb_get( 'pwa_background_color', '#ffffff' ) : '#ffffff' ),
		'start_url'   => $start,
		'scope_path'  => $scope,
	];
	return apply_filters( 'home_base_pwa_config', $cfg );
}
...
add_action( 'init', function () {
	...
	if ( $path === '' ) return;
	$base = basename( $path );
	if ( $base === HOME_BASE_PWA_MANIFEST_FILE ) hb_pwa_render_manifest();
	if ( $base === HOME_BASE_PWA_SW_FILE )       hb_pwa_render_sw();
}, 0 );

function hb_pwa_render_manifest(): void {
	$cfg = hb_pwa_config();
 ...
 */
function hb_pwa_icons(): array {
	$url = hb_pwa_source_url();
	if ( $url === '' ) return [];
...
function hb_pwa_source_url(): string {
	$explicit = (string) ( function_exists( 'hb_get' ) ? hb_get( 'pwa_icon_url', '' ) : '' );
...
function hb_pwa_render_sw(): void {
	$cfg       = hb_pwa_config();
	$cache_ver = HOME_BASE_PWA_VERSION;
	$app_name  = $cfg['name'];
	...
	?>
/* Home Base service worker — v<?php echo $cache_ver; ?> */
'use strict';

const HB_CACHE = 'home-base-v<?php echo esc_js( $cache_ver ); ?>';
const OFFLINE_HTML = <?php echo $offline_json; ?>;
const HB_HOME = '<?php echo esc_js( $cfg['start_url'] ); ?>';
...
function isBypass(url, req) {
	...
	return p.indexOf('/wp-admin/') === 0
		...
		|| p.indexOf('<?php echo esc_js( HOME_BASE_PWA_SW_FILE ); ?>') !== -1
		|| p.indexOf('<?php echo esc_js( HOME_BASE_PWA_MANIFEST_FILE ); ?>') !== -1;
}
...
self.addEventListener('activate', (e) => {
	e.waitUntil((async () => {
		...
		await Promise.all(keys.filter((k) => k !== HB_CACHE).map((k) => caches.delete(k)));
...
self.addEventListener('fetch', (event) => {
	...
	if (url.origin === self.location.origin && ASSET_RE.test(url.pathname)) {
		event.respondWith((async () => {
			const cache = await caches.open(HB_CACHE);
...
self.addEventListener('push', (event) => {
	let data = { title: '<?php echo esc_js( $app_name ); ?>', body: 'You have a new update.', url: HB_HOME };
	try { if (event.data) data = Object.assign(data, event.data.json()); } catch (_) {}
	event.waitUntil((async () => {
		await self.registration.showNotification(data.title, {
			body: data.body, tag: data.tag || 'home-base', renotify: true,
			data: { url: data.url || HB_HOME }
		});
...
self.addEventListener('notificationclick', (event) => {
	...
	const target = (event.notification.data && event.notification.data.url) || HB_HOME;
	event.waitUntil((async () => {
...
add_action( 'wp_head', 'hb_pwa_head', 5 );

function hb_pwa_head(): void {
	if ( ! function_exists( 'hb_is_app_page' ) || ! hb_is_app_page() ) return;

	$cfg          = hb_pwa_config();
	$manifest_url = home_url( '/' . HOME_BASE_PWA_MANIFEST_FILE );
	$sw_url       = home_url( '/' . HOME_BASE_PWA_SW_FILE );
	$apple_icon   = hb_pwa_source_url();
	...
	(function () {
		var HB = <?php echo $data; ?>;
		if ('serviceWorker' in navigator) {
			window.addEventListener('load', function () {
				navigator.serviceWorker.register(HB.sw, { scope: HB.scope }).catch(function () {});
			});
		}
		var standalone = window.matchMedia('(display-mode: standalone)').matches || window.navigator.standalone === true;
		if (standalone) return;

		var DISMISS_KEY = 'hb_pwa_install_dismissed';
		...
		function showBanner(html, onAction) {
			if (dismissed() || document.getElementById('hb-install-banner')) return;
			var bar = document.createElement('div');
			bar.id = 'hb-install-banner';
			bar.setAttribute('role', 'dialog');
			bar.innerHTML = html +
				'<div class="hb-ib-actions">' +
				(onAction ? '<button type="button" class="hb-ib-go">Install</button>' : '') +
				'<button type="button" class="hb-ib-x" aria-label="Dismiss">Not now</button></div>';
			var s = document.createElement('style');
			s.textContent =
				'#hb-install-banner{position:fixed;left:12px;right:12px;bottom:12px;z-index:99999;' +
				...
				'#hb-install-banner .hb-ib-title{font-weight:700;font-size:15px;color:#1f2933}' +
				'#hb-install-banner .hb-ib-text{font-size:13px;line-height:1.45;color:#52606d}' +
				'#hb-install-banner .hb-ib-actions{display:flex;gap:8px;justify-content:flex-end}' +
				'#hb-install-banner button{font:inherit;font-weight:600;border-radius:9px;padding:9px 16px;border:0;cursor:pointer}' +
				'#hb-install-banner .hb-ib-go{background:<?php echo esc_js( $cfg['theme_color'] ); ?>;color:#fff}' +
				'#hb-install-banner .hb-ib-x{background:#eef1f4;color:#52606d}';
			document.head.appendChild(s);
			document.body.appendChild(bar);
			bar.querySelector('.hb-ib-x').addEventListener('click', function () { setDismissed(); bar.remove(); });
			var go = bar.querySelector('.hb-ib-go');
		...
		window.addEventListener('beforeinstallprompt', function (e) {
			...
			showBanner(
				'<div class="hb-ib-title">Install ' + HB.name + '</div>' +
				'<div class="hb-ib-text">Add ' + HB.name + ' to your home screen for one-tap access.</div>',
				function (bar) {
		...
		if (isIOS && isSafari) {
			window.addEventListener('load', function () {
				setTimeout(function () {
					showBanner(
						'<div class="hb-ib-title">Install ' + HB.name + '</div>' +
						'<div class="hb-ib-text">Tap the Share button, then <strong>Add to Home Screen</strong>.</div>',
						null
					);
				}, 1500);
			});
