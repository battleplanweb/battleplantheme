...
function battleplan_carte_du_jour_scripts() {
	bp_inline_minified_css( get_template_directory() . '/style-carte-du-jour.css' );
	bp_enqueue_script( 'battleplan-script-carte-du-jour', 'script-carte-du-jour' );
}
...
function battleplan_carte_du_jour_phone() {
	...
	foreach ( get_option('cdj_locations') as $loc ) :
		$buildPhone .= '<a href="tel:1-'.$loc['phone'].'" class="show-'.$loc['slug'].' phone-link track-clicks" data-action="phone-call"><div class="mm-bar-btn mm-bar-phone call-btn" aria-hidden="true"></div><span class="sr-only">Call Us</span></a>';
	endforeach;
	$buildPhone .= '</div>';
