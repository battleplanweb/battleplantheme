...
require_once get_template_directory() . '/includes/includes-site-pulse-card-import.php';
...
// "Is the god all-access bypass active right now?" — true only when the real user is a god AND is NOT
// impersonating. Every permission/visibility bypass uses this (not raw is_god) so that while a god is
// viewing-as another user, every gate falls through to that user's real capabilities — the god sees and
// can do EXACTLY what that user can, nothing more. (Stopping impersonation is gated on the real god id
// separately — the god bar and the impersonate handler — so the god can always exit.)
function site_pulse_god_can_override(): bool {
	return site_pulse_is_god( (int) get_current_user_id() ) && ! site_pulse_is_impersonating();
}
...
add_action( 'init', function() {
	...
	// Daily GM/Supervisor report-due reminder cron (self-gates on the Reports schedule settings).
	if ( ! wp_next_scheduled( 'site_pulse_report_due_reminder' ) ) {
		wp_schedule_event( strtotime( 'tomorrow 7:00am' ), 'daily', 'site_pulse_report_due_reminder' );
	}
...
add_action( 'template_redirect', function() {
	...
	add_filter( 'body_class', function( $classes ) {
		...
		// The god bar renders for a REAL god (even while impersonating, so it can't key off the
		// impersonated sp-role-* class above). This one class drives the whole-layout god-bar offset
		// (CSS var --sp-god-h) AND the bar's own visibility — remove it to preview the non-god layout.
		if ( site_pulse_is_god( (int) get_current_user_id() ) ) $classes[] = 'sp-has-god-bar';
		return $classes;
	} );
} );
...
function site_pulse_enqueue_assets(): void {
	...
	// Per-user remembered Action Items sort order (importance | duedate | custom). On the real user.
	$action_sort        = get_user_meta( $real_user_id, '_sp_action_sort', true );
	$action_sort        = in_array( $action_sort, [ 'importance', 'duedate', 'custom' ], true ) ? $action_sort : 'importance';
	$location_name      = '';
	...
	wp_localize_script( 'site-pulse-script', 'sitePulseData', [
		...
		// God power for the JS UI is suppressed while impersonating, so "view as <user>" hides every
		// god-only control just like that user sees. (The god bar/picker is server-rendered for the real
		// god, so the god can still switch/stop.)
		'isGod'              => site_pulse_god_can_override(),
		...
		// Per-user chart style for the distribution graphs (bar | pie). Remembered on the real user.
...
function site_pulse_submit_report( int $report_id ): bool {
	...
	if ( $updated ) {
		...
		if ( $report ) {
			...
			// One "Report submitted" event for every report type. The contextual recipients adapt to
			// who submitted: User = the submitter, User's Supervisor = their direct supervisor — so a
			// GM's report notifies their Supervisor, a Supervisor's report notifies their Owner, etc.
			site_pulse_dispatch_notification( 'report_submitted', (int) $report['user_id'], $msg, $report_id, 'report' );
		}
	}
...
// Also stamp on every login — guarantees a sign-in is recorded even if the dashboard is served from
// cache and the user leaves before the SPA fires its first poll.
add_action( 'wp_login', function ( $login, $user ) {
	if ( $user instanceof WP_User && $user->ID ) update_user_meta( $user->ID, '_sp_last_active', time() );
}, 10, 2 );
...
// Remember a user's chart style (bar | pie) for the distribution graphs. Stored on the REAL user so
// a god impersonating someone doesn't overwrite that person's preference.
add_action( 'wp_ajax_site_pulse_set_chart_style', 'site_pulse_ajax_set_chart_style' );
function site_pulse_ajax_set_chart_style(): void {
	check_ajax_referer( 'site_pulse_nonce', 'nonce' );
	$uid = get_current_user_id();
	if ( ! $uid ) wp_send_json_error();
	$style = ( ( $_POST['style'] ?? '' ) === 'pie' ) ? 'pie' : 'bar';
	update_user_meta( $uid, '_sp_chart_style', $style );
	wp_send_json_success( [ 'style' => $style ] );
}

// Remember the user's Action Items sort order so it loads that way next time.
add_action( 'wp_ajax_site_pulse_set_action_sort', 'site_pulse_ajax_set_action_sort' );
function site_pulse_ajax_set_action_sort(): void {
	check_ajax_referer( 'site_pulse_nonce', 'nonce' );
	$uid = get_current_user_id();
	if ( ! $uid ) wp_send_json_error();
	$sort = in_array( $_POST['sort'] ?? '', [ 'importance', 'duedate', 'custom' ], true ) ? $_POST['sort'] : 'importance';
	update_user_meta( $uid, '_sp_action_sort', $sort );
...
function site_pulse_ajax_admin_get_templates(): void {
	...
	wp_send_json_success( [
		'templates' => $templates,
		'roles'     => site_pulse_get_all_roles(),
		'schedule'  => [
			'gm_frequency'         => site_pulse_get_setting( 'gm_report_frequency', '' ),
			'gm_anchor'            => site_pulse_get_setting( 'gm_report_anchor', '' ),
			'supervisor_frequency' => site_pulse_get_setting( 'supervisor_report_frequency', '' ),
			'supervisor_anchor'    => site_pulse_get_setting( 'supervisor_report_anchor', '' ),
		],
	] );
}

// Save the GM/Supervisor report-due schedule (frequency + starting due date). Powers the daily
// site_pulse_report_due_reminder cron (a "report due today" notification on each due date).
add_action( 'wp_ajax_site_pulse_save_report_schedule', 'site_pulse_ajax_save_report_schedule' );
function site_pulse_ajax_save_report_schedule(): void {
	check_ajax_referer( 'site_pulse_nonce', 'nonce' );
	if ( ! site_pulse_admin_check( 'manage_templates' ) ) return;

	global $wpdb;
	$valid    = [ '', 'weekly', 'biweekly', 'monthly' ];
	$slug_map = [ 'gm' => 'manager-biweekly', 'supervisor' => 'supervisor-biweekly' ]; // fixed report set
	$prefix   = [ 'gm' => 'GM', 'supervisor' => 'Supervisor' ];
	$tpl_tbl  = site_pulse_table( 'report_templates' );

	foreach ( [ 'gm', 'supervisor' ] as $key ) {
		$freq = sanitize_key( wp_unslash( $_POST[ $key . '_frequency' ] ?? '' ) );
		if ( ! in_array( $freq, $valid, true ) ) $freq = '';
		$anchor = sanitize_text_field( wp_unslash( $_POST[ $key . '_anchor' ] ?? '' ) );
		if ( ! preg_match( '/^\d{4}-\d{2}-\d{2}$/', $anchor ) ) $anchor = '';
		site_pulse_set_setting( $key . '_report_frequency', $freq );
		site_pulse_set_setting( $key . '_report_anchor', $anchor );

		// (continued below) Make this the single source of truth for the cadence: sync the report template's
		// frequency, and its name's frequency word — unless the admin gave it a non-standard name.
		if ( '' !== $freq ) {
			$tpl = $wpdb->get_row( $wpdb->prepare( "SELECT id, name FROM $tpl_tbl WHERE slug = %s", $slug_map[ $key ] ), ARRAY_A );
			if ( $tpl ) {
				$update = [ 'frequency' => $freq ];
				$label  = site_pulse_frequency_label( $freq );
				if ( $label && preg_match( '/^' . preg_quote( $prefix[ $key ], '/' ) . ' .+ Report$/', (string) $tpl['name'] ) ) {
					$update['name'] = $prefix[ $key ] . ' ' . $label . ' Report';
				}
				$wpdb->update( $tpl_tbl, $update, [ 'id' => (int) $tpl['id'] ] );
			}
		}
	}
	wp_send_json_success();
...
// One-time migration for the consolidated notifications model: (a) backfill 'bell' => 1 into every
// previously-saved routing row (the bell was implicitly always-on before it became a column), and
// (b) seed the new global email switch from the old per-message email flag.
add_action( 'init', 'site_pulse_migrate_notifications_v2' );
function site_pulse_migrate_notifications_v2(): void {
	if ( get_option( 'site_pulse_notif_v2_seeded' ) ) return;

	$raw = site_pulse_get_setting( 'notification_routing', '' );
	if ( '' !== $raw ) {
		$saved = json_decode( $raw, true );
		if ( is_array( $saved ) ) {
			foreach ( $saved as $ev => &$cols ) {
				if ( is_array( $cols ) && ! empty( $cols ) ) $cols['bell'] = 1; // had a bell before
			}
			unset( $cols );
			site_pulse_set_setting( 'notification_routing', wp_json_encode( $saved ) );
		}
	}

	if ( '' === site_pulse_get_setting( 'notifications_email_enabled', '' ) ) {
		$legacy = site_pulse_get_setting( 'messages_email_enabled', '0' );
		site_pulse_set_setting( 'notifications_email_enabled', '1' === $legacy ? '1' : '0' );
	}

	update_option( 'site_pulse_notif_v2_seeded', '1' );
}

// One-time: convert the old fixed role-broadcast keys to the dynamic 'role:<slug>' keys so saved
// routing keeps working after the recipient columns became role-driven.
add_action( 'init', 'site_pulse_migrate_notifications_v3' );
function site_pulse_migrate_notifications_v3(): void {
	if ( get_option( 'site_pulse_notif_v3_seeded' ) ) return;

	$map = [ 'all_gms' => 'role:manager', 'all_supervisors' => 'role:supervisor', 'admin' => 'role:admin', 'managers' => 'role:non-store-manager', 'owners' => 'role:owner' ];
	$raw = site_pulse_get_setting( 'notification_routing', '' );
	if ( '' !== $raw ) {
		$saved = json_decode( $raw, true );
		if ( is_array( $saved ) ) {
...
function site_pulse_notification_events(): array {
	return [
		...
		'action_urgent'               => 'High-priority action item created',
...
// The matrix columns. The first group are DELIVERY channels (how it's sent): 'bell' = an in-app
// notification-bell entry; 'dashboard' = additionally a dismissible banner (a flagged bell entry, so
// Dashboard implies Bell); 'email' = also emailed; 'push' = also a device push. Email/Push obey the
// app-level switches in Settings → Notifications. The rest are RECIPIENTS (who): 'gm' = the event's
// subject person; 'supervisor' = that person's direct supervisor; the others broadcast to everyone
// holding that Site Pulse role.
function site_pulse_notification_columns(): array {
	// Delivery channels + the two contextual recipients are fixed. The role-broadcast columns are
	// generated from Settings → Roles (key 'role:<slug>', label = the role's label), so adding,
	// renaming, or removing a role flows straight through to this matrix. 'god' is excluded.
	$cols = [
		'dashboard'  => 'Dash',   // short header to save column width; help text still says "Dashboard"
		'email'      => 'Email',
		'push'       => 'Push',
		'bell'       => 'Bell',
		'gm'         => 'User',
	...
	// Roles listed in reverse (last role → role 1): site_pulse_get_all_roles() returns hierarchy
	// DESC, so array_reverse() puts the lowest role first here without disturbing that shared order.
	foreach ( array_reverse( site_pulse_get_all_roles() ) as $r ) {
		$slug = (string) ( $r['slug'] ?? '' );
		if ( '' === $slug ) continue;
		$cols[ 'role:' . $slug ] = (string) ( $r['label'] ?? $slug );
	}
	return $cols;
}
...
function site_pulse_notification_defaults(): array {
	return [
		...
		'action_followup'             => [ 'bell' => 1, 'gm' => 1, 'supervisor' => 1, 'push' => 1 ],
		'action_resolved'             => [ 'bell' => 1, 'supervisor' => 1, 'push' => 1 ],
		'action_urgent'               => [ 'bell' => 1, 'supervisor' => 1, 'push' => 1 ],
		'period_ending_2days'         => [ 'bell' => 1, 'gm' => 1, 'push' => 1 ],
		'period_ending_tomorrow'      => [ 'bell' => 1, 'gm' => 1, 'push' => 1 ],
		'period_ends_today'           => [ 'bell' => 1, 'gm' => 1, 'push' => 1 ],
...
function site_pulse_dispatch_notification( string $event, int $subject_user_id, string $message, int $related_id = 0, string $related_type = '' ): void {
	...
	// Channel flags (not recipients). 'bell' = create an in-app notification entry; 'dashboard' elevates
	// that entry to a banner (so dashboard implies a bell row); 'email' also emails (gated by the app
	// switch); 'push' fires a Web Push wake (push_send self-gates on the app switch).
	$on_dashboard = ! empty( $cols['dashboard'] );
	$on_bell      = ! empty( $cols['bell'] );
	$do_push      = ! empty( $cols['push'] );
	$on_email     = ! empty( $cols['email'] ) && site_pulse_get_setting( 'notifications_email_enabled', '0' ) === '1';
	...
	// Role-broadcast columns ('role:<slug>'), plus back-compat for the pre-dynamic fixed keys.
	$legacy_role_cols = [ 'all_gms' => 'manager', 'all_supervisors' => 'supervisor', 'admin' => 'admin', 'managers' => 'non-store-manager', 'owners' => 'owner' ];
	foreach ( $cols as $ckey => $on ) {
		if ( empty( $on ) ) continue;
		$slug = ( 0 === strpos( $ckey, 'role:' ) ) ? substr( $ckey, 5 ) : ( $legacy_role_cols[ $ckey ] ?? '' );
		if ( '' !== $slug ) $recipients = array_merge( $recipients, site_pulse_user_ids_with_role( $slug ) );
	}

	$recipients = array_values( array_unique( array_map( 'intval', array_filter( $recipients ) ) ) );
	// The dashboard banner is stored as a (flagged) bell row, so create a row when EITHER is on.
	$make_row = $on_bell || $on_dashboard;
	foreach ( $recipients as $rid ) {
		if ( $make_row )                                          site_pulse_notify( $rid, $event, $message, $related_id, $related_type, $on_dashboard );
		if ( $do_push && function_exists( 'site_pulse_push_send' ) ) site_pulse_push_send( $rid );
		if ( $on_email )                                          site_pulse_notify_email( $rid, $message );
	}
}

// Email one recipient a plain notification (used by the matrix' Email channel + new-message emails).
function site_pulse_notify_email( int $user_id, string $message ): void {
	$u = get_userdata( $user_id );
	if ( ! $u || ! is_email( $u->user_email ) ) return;
	$site = wp_specialchars_decode( get_bloginfo( 'name' ), ENT_QUOTES );
	wp_mail( $u->user_email, $site . ' — notification', $message . "\n\n" . home_url( '/' ) );
}
...
function site_pulse_ajax_get_notification_settings(): void {
	...
	wp_send_json_success( [
		...
		'email_enabled' => site_pulse_get_setting( 'notifications_email_enabled', '0' ),
...
// Human label for a report frequency value. '' for unknown/unset.
function site_pulse_frequency_label( string $freq ): string {
	$map = [ 'weekly' => 'Weekly', 'biweekly' => 'Bi-Weekly', 'monthly' => 'Monthly' ];
	return $map[ $freq ] ?? '';
}

// Is today a report due date, given a frequency (weekly | biweekly | monthly) and a starting due date?
// Periods tile forward from the anchor — like the expense period, but driven by a frequency.
function site_pulse_report_is_due_today( string $frequency, string $anchor ): bool {
	if ( ! preg_match( '/^\d{4}-\d{2}-\d{2}$/', $anchor ) ) return false;
	$anchor_ts = strtotime( $anchor . ' 00:00:00' );
	$today_ts  = strtotime( current_time( 'Y-m-d' ) . ' 00:00:00' );
	if ( $anchor_ts === false || $today_ts === false || $today_ts < $anchor_ts ) return false;

	if ( 'monthly' === $frequency ) {
		// Same day-of-month as the anchor, clamped to the month's length (e.g. a 31st anchor → 30th/28th).
		$want = (int) gmdate( 'j', $anchor_ts );
		$dim  = (int) gmdate( 't', $today_ts );
		return (int) gmdate( 'j', $today_ts ) === min( $want, $dim );
	}

	$step = ( 'weekly' === $frequency ) ? 7 : ( ( 'biweekly' === $frequency ) ? 14 : 0 );
	if ( $step < 1 ) return false;
	$days = (int) round( ( $today_ts - $anchor_ts ) / 86400 );
	return ( $days % $step ) === 0;
}
...
// Past-due reports: GMs / Supervisors whose last SUBMITTED report is older than their report
// frequency (or who've never submitted while a schedule is configured). Powers the dashboard widget.
function site_pulse_reports_past_due(): array {
	global $wpdb;
	$now   = current_time( 'timestamp' );
	$out   = [];
	$types = [
		'manager'    => [ 'freq' => site_pulse_get_setting( 'gm_report_frequency', '' ),         'role' => 'GM' ],
		'supervisor' => [ 'freq' => site_pulse_get_setting( 'supervisor_report_frequency', '' ), 'role' => 'Supervisor' ],
	];
	foreach ( $types as $slug => $cfg ) {
		$freq = $cfg['freq'];
		if ( '' === $freq ) continue;   // that report type isn't on a schedule → nobody's "past due"
		$cutoff = strtotime( ( 'weekly' === $freq ? '-7 days' : ( 'biweekly' === $freq ? '-14 days' : '-1 month' ) ), $now );
		foreach ( site_pulse_user_ids_with_role( $slug ) as $uid ) {
			// Only people whose "Submit reports" box is actually checked (role default or per-user
			// override). has_cap_raw — NOT user_can — so a god doesn't bypass it, and the reports
			// module being on/off (already gated at the widget level) doesn't change the roster.
			if ( ! site_pulse_has_cap_raw( $uid, 'submit_reports' ) ) continue;
			$profile = site_pulse_get_user_profile( $uid );
			if ( ! $profile || ( $profile['status'] ?? 'active' ) !== 'active' ) continue;
			$last    = $wpdb->get_var( $wpdb->prepare(
				"SELECT MAX(submitted_at) FROM " . site_pulse_table('reports') . " WHERE user_id = %d AND status = 'submitted'",
				$uid
			) );
			$last_ts = $last ? strtotime( $last ) : 0;
			if ( $last_ts && $last_ts >= $cutoff ) continue;   // submitted recently enough → not past due
			$u   = get_userdata( $uid );
			$loc = site_pulse_get_location( (int) ( $profile['location_id'] ?? 0 ) );
			$out[] = [
				'user_id'    => $uid,
				'name'       => $u ? $u->display_name : ( 'User #' . $uid ),
				'role'       => $cfg['role'],
				'location'   => $loc ? $loc['name'] : '',
				'last_label' => $last_ts ? ( human_time_diff( $last_ts, $now ) . ' ago' ) : 'Never submitted',
				'last'       => $last ?: '',
			];
		}
	}
	// Most overdue first: never-submitted (empty 'last') sort ahead of the oldest dates.
	usort( $out, function( $a, $b ) { return strcmp( $a['last'], $b['last'] ); } );
	return $out;
}

// Who sees the past-due widget — driven by the role-group columns of the 'reports_past_due' row in
// Settings → Notifications (All GMs / All Supervisors / Admin / Managers / Owners). God always sees it.
function site_pulse_user_sees_pastdue_widget( int $user_id ): bool {
	if ( function_exists( 'site_pulse_is_god' ) && site_pulse_is_god( $user_id ) ) return true;
	if ( function_exists( 'site_pulse_module_on' ) && ! site_pulse_module_on( 'reports' ) ) return false;
	$profile = site_pulse_get_user_profile( $user_id );
	if ( ! $profile ) return false;
	$role = site_pulse_get_role( (int) $profile['role_id'] );
	$slug = $role['slug'] ?? '';
	...
	$routing = site_pulse_get_notification_routing();
	return ! empty( $routing['reports_past_due'][ 'role:' . $slug ] );
}

add_action( 'wp_ajax_site_pulse_pastdue_reports', 'site_pulse_ajax_pastdue_reports' );
function site_pulse_ajax_pastdue_reports(): void {
	check_ajax_referer( 'site_pulse_nonce', 'nonce' );
	$user_id = site_pulse_effective_user_id();
	if ( ! site_pulse_user_sees_pastdue_widget( $user_id ) ) wp_send_json_error( [ 'message' => 'Not permitted.' ] );
	wp_send_json_success( [ 'list' => site_pulse_reports_past_due() ] );
}
...
function site_pulse_call_claude( string $prompt, string $system = '', array $opts = [], &$debug = null ): ?string {
	...
	// Multi-turn callers (the AI Assistant) pass a full messages array via opts; everyone else sends a
	// single user prompt.
	$messages = ( ! empty( $opts['messages'] ) && is_array( $opts['messages'] ) )
		? $opts['messages']
...
// Default roles, used until an admin customizes the list (Settings → AI Roles). Each = name + persona.
function site_pulse_ai_assistant_default_roles(): array {
	return [
		[ 'name' => 'Social Media Manager',  'persona' => 'You craft engaging social posts (Facebook, Instagram). Hook fast, keep it skimmable, end with a clear call to action, and add a few relevant hashtags when useful.' ],
		[ 'name' => 'Ad Copy Writer',        'persona' => 'You write persuasive, benefit-led ad and promotional copy. Lead with the offer, create a little urgency, and finish with a strong CTA. Offer 2-3 variations unless told otherwise.' ],
		[ 'name' => 'Customer Service Agent', 'persona' => 'You write warm, professional, solution-focused customer replies. Acknowledge, empathize, resolve, and keep it concise.' ],
		[ 'name' => 'Email Writer',          'persona' => 'You write clear, friendly business emails — a strong subject line, a brief body, and an obvious next step.' ],
		[ 'name' => 'General Assistant',     'persona' => 'You are a helpful, versatile business assistant.' ],
	];
}

// The configured role list (or defaults if none saved).
function site_pulse_ai_assistant_roles(): array {
	$raw = site_pulse_get_setting( 'ai_assistant_roles', '' );
	$arr = $raw ? json_decode( $raw, true ) : null;
	if ( ! is_array( $arr ) || ! $arr ) return site_pulse_ai_assistant_default_roles();
	$out = [];
	foreach ( $arr as $r ) {
		if ( ! is_array( $r ) ) continue;
		$n = trim( (string) ( $r['name'] ?? '' ) );
		if ( '' === $n ) continue;
		$out[] = [ 'name' => $n, 'persona' => trim( (string) ( $r['persona'] ?? '' ) ) ];
	}
	return $out ?: site_pulse_ai_assistant_default_roles();
}

// The assistant's display name (what employees see in the nav/header). Defaults to "AI Assistant";
// a company can rename it (e.g. "bAIbes") in Settings → AI Roles.
function site_pulse_ai_assistant_name(): string {
	$n = trim( (string) site_pulse_get_setting( 'ai_assistant_name', '' ) );
	return '' !== $n ? $n : 'AI Assistant';
}

// System prompt = role persona + company brand voice + "interpret casual requests like a pro" guidance.
function site_pulse_ai_assistant_system( string $role_name, string $persona ): string {
	$company = trim( (string) site_pulse_get_setting( 'company_name', '' ) );
	$voice   = function_exists( 'site_pulse_get_company_voice' ) ? trim( site_pulse_get_company_voice( '' ) ) : '';
	$who     = '' !== $company ? "for {$company}" : 'for this company';

	$s  = 'You are an expert ' . ( '' !== $role_name ? $role_name : 'assistant' ) . " {$who}. ";
	if ( '' !== $persona ) $s .= $persona . ' ';
	$s .= "\n\nProduce polished, ready-to-use output. Employees often type brief or casual requests — interpret them generously like a skilled colleague would, make reasonable assumptions, and deliver something usable rather than asking a lot of clarifying questions. Only ask a question when the request genuinely can't proceed without it.";
	if ( '' !== $voice ) {
		$s .= "\n\nAlways write in this company's brand voice and follow these guidelines:\n" . $voice;
	}
	return $s;
}

// AJAX — roles for the assistant's dropdown (anyone who can use the assistant).
add_action( 'wp_ajax_site_pulse_ai_assistant_roles', 'site_pulse_ajax_ai_assistant_roles' );
function site_pulse_ajax_ai_assistant_roles(): void {
	check_ajax_referer( 'site_pulse_nonce', 'nonce' );
	$uid = site_pulse_effective_user_id();
	if ( ! ( site_pulse_user_can( $uid, 'use_ai_assistant' ) || site_pulse_god_can_override() ) ) wp_send_json_error( [ 'message' => 'Not authorized.' ] );
	wp_send_json_success( [ 'roles' => site_pulse_ai_assistant_roles(), 'name' => site_pulse_ai_assistant_name() ] );
}

// AJAX — one chat turn. Takes the role + the running conversation; returns the assistant's reply.
add_action( 'wp_ajax_site_pulse_ai_assistant_send', 'site_pulse_ajax_ai_assistant_send' );
function site_pulse_ajax_ai_assistant_send(): void {
	check_ajax_referer( 'site_pulse_nonce', 'nonce' );
	$uid = site_pulse_effective_user_id();
	if ( ! ( site_pulse_user_can( $uid, 'use_ai_assistant' ) || site_pulse_god_can_override() ) ) wp_send_json_error( [ 'message' => 'Not authorized.' ] );
	if ( ! site_pulse_get_api_key() ) wp_send_json_error( [ 'message' => 'No AI key is configured for this site.' ] );

	$role_name = sanitize_text_field( wp_unslash( $_POST['role'] ?? '' ) );
	$raw_msgs  = json_decode( wp_unslash( $_POST['messages'] ?? '' ), true );
	if ( ! is_array( $raw_msgs ) || ! $raw_msgs ) wp_send_json_error( [ 'message' => 'Nothing to send.' ] );

	// Sanitize the conversation: only user/assistant turns, non-empty, capped length + history depth.
	$messages = [];
	foreach ( array_slice( $raw_msgs, -40 ) as $m ) {
		$role = ( ( $m['role'] ?? '' ) === 'assistant' ) ? 'assistant' : 'user';
		$text = trim( (string) ( $m['content'] ?? '' ) );
		if ( '' === $text ) continue;
		$messages[] = [ 'role' => $role, 'content' => mb_substr( $text, 0, 8000 ) ];
	}
	$lastMsg = $messages ? end( $messages ) : null;
	if ( ! $lastMsg || 'user' !== $lastMsg['role'] ) wp_send_json_error( [ 'message' => 'Nothing to send.' ] );

	// Resolve the role's persona (blank for a custom-typed role — the name alone still steers the system).
	$persona = '';
	foreach ( site_pulse_ai_assistant_roles() as $r ) {
		if ( strcasecmp( $r['name'], $role_name ) === 0 ) { $persona = $r['persona']; break; }
	}
	$system = site_pulse_ai_assistant_system( $role_name, $persona );

	$dbg   = null;
	$reply = site_pulse_call_claude( '', $system, [ 'messages' => $messages, 'max_tokens' => 2048 ], $dbg );
	if ( null === $reply ) wp_send_json_error( [ 'message' => $dbg ?: 'The AI did not respond — please try again.' ] );

	site_pulse_log( 'ai_assistant', 'AI Assistant used', [ 'role' => $role_name ] );
	wp_send_json_success( [ 'reply' => $reply ] );
}

// AJAX — admin get/save the editable role list (Settings → AI Roles).
add_action( 'wp_ajax_site_pulse_get_ai_roles', 'site_pulse_ajax_get_ai_roles' );
function site_pulse_ajax_get_ai_roles(): void {
	check_ajax_referer( 'site_pulse_nonce', 'nonce' );
	if ( ! site_pulse_admin_check( 'manage_settings' ) ) return;
	wp_send_json_success( [ 'roles' => site_pulse_ai_assistant_roles(), 'name' => site_pulse_ai_assistant_name() ] );
}
add_action( 'wp_ajax_site_pulse_save_ai_roles', 'site_pulse_ajax_save_ai_roles' );
function site_pulse_ajax_save_ai_roles(): void {
	check_ajax_referer( 'site_pulse_nonce', 'nonce' );
	if ( ! site_pulse_admin_check( 'manage_settings' ) ) return;
	if ( isset( $_POST['name'] ) ) {
		site_pulse_set_setting( 'ai_assistant_name', mb_substr( trim( sanitize_text_field( wp_unslash( $_POST['name'] ) ) ), 0, 40 ) );
	}
	$in  = json_decode( wp_unslash( $_POST['roles'] ?? '' ), true );
	$out = [];
	if ( is_array( $in ) ) {
		foreach ( $in as $r ) {
			if ( ! is_array( $r ) ) continue;
			$n = trim( (string) ( $r['name'] ?? '' ) );
			if ( '' === $n ) continue;
			$out[] = [ 'name' => mb_substr( $n, 0, 60 ), 'persona' => mb_substr( trim( (string) ( $r['persona'] ?? '' ) ), 0, 1500 ) ];
		}
	}
	site_pulse_set_setting( 'ai_assistant_roles', wp_json_encode( $out ) );
	wp_send_json_success( [ 'roles' => site_pulse_ai_assistant_roles(), 'name' => site_pulse_ai_assistant_name() ] );
}
...
function site_pulse_ajax_create_action_item(): void {
	...
	if ( '' === $cat ) $cat = 'To-Do'; // blank → keep the default label
	...
	// Me first, then the additional people. >1 assignee → a shared group_id so completing/reopening one
	// copy syncs the whole set (same model the chat-created multi-assign items use).
	$assignees = array_values( array_unique( array_merge( [ $me ], array_filter( $extra, fn( $u ) => $u !== $me ) ) ) );
	$multi     = count( $assignees ) > 1;
	$group_id  = $multi ? wp_generate_password( 20, false ) : null;
	...
	foreach ( $assignees as $uid ) {
		...
		if ( $uid === $me ) {
			...
			// Notify the added person (bell + their devices), mirroring the chat→action flow.
	...
	wp_send_json_success( [ 'id' => $my_id ] );
}


// Save a user's dashboard widget layout (order + hidden set). Stored per-user; only known widget ids
// are persisted, so stale/unknown ids can't accumulate.
add_action( 'wp_ajax_site_pulse_save_dashboard_layout', 'site_pulse_ajax_save_dashboard_layout' );
function site_pulse_ajax_save_dashboard_layout(): void {
	check_ajax_referer( 'site_pulse_nonce', 'nonce' );
	$uid = site_pulse_effective_user_id();
	if ( ! $uid ) wp_send_json_error( [ 'message' => 'Not signed in.' ] );

	$known  = [ 'alerts', 'chat', 'actions', 'reports', 'mileage', 'feedback', 'directory' ];
	$order  = array_values( array_intersect( array_map( 'sanitize_text_field', (array) ( $_POST['order'] ?? [] ) ), $known ) );
	$hidden = array_values( array_intersect( array_map( 'sanitize_text_field', (array) ( $_POST['hidden'] ?? [] ) ), $known ) );

	update_user_meta( $uid, '_sp_dashboard_layout', wp_json_encode( [ 'order' => $order, 'hidden' => $hidden ] ) );
	wp_send_json_success();
}

// Per-channel feedback counts for the Customer Feedback dashboard widget. Each channel is included
// only if the viewer can access it (so the widget never leaks an area they lack permission for).
add_action( 'wp_ajax_site_pulse_feedback_summary', 'site_pulse_ajax_feedback_summary' );
function site_pulse_ajax_feedback_summary(): void {
	check_ajax_referer( 'site_pulse_nonce', 'nonce' );
	$uid = site_pulse_effective_user_id();
	if ( ! $uid ) wp_send_json_error( [ 'message' => 'Not signed in.' ] );

	global $wpdb;
	$can    = function ( $c ) use ( $uid ) { return site_pulse_user_can( $uid, $c ); };
	$cutoff = gmdate( 'Y-m-d H:i:s', current_time( 'timestamp', true ) - 30 * DAY_IN_SECONDS );
	$out    = [];

	if ( ( $can( 'view_reviews' ) || $can( 'manage_reviews' ) ) && function_exists( 'sp_reviews_table' ) ) {
		$rt  = sp_reviews_table();
		$cnt = (int) $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM $rt WHERE create_time >= %s", $cutoff ) );
		$avg = $wpdb->get_var( $wpdb->prepare( "SELECT AVG(star_rating) FROM $rt WHERE create_time >= %s AND star_rating BETWEEN 1 AND 5", $cutoff ) );
		$out['reviews'] = [ 'count' => $cnt, 'avg' => ( null !== $avg ? round( (float) $avg, 1 ) : null ) ];
	}
	if ( $can( 'view_surveys' ) || $can( 'manage_surveys' ) ) {
		$st = site_pulse_table( 'surveys' );
		$out['surveys'] = [ 'count' => (int) $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM $st WHERE created_at >= %s", $cutoff ) ) ];
	}
	if ( ( $can( 'view_customer_messages' ) || $can( 'manage_customer_messages' ) ) && function_exists( 'sp_cm_table' ) ) {
		$out['messages'] = [ 'unread' => (int) $wpdb->get_var( "SELECT COUNT(*) FROM " . sp_cm_table( 'cm_conversations' ) . " WHERE unread = 1" ) ];
	}
	if ( ( $can( 'view_emails' ) || $can( 'manage_emails' ) ) && function_exists( 'sp_emails_table' ) ) {
		$out['emails'] = [ 'new' => (int) $wpdb->get_var( "SELECT COUNT(*) FROM " . sp_emails_table() . " WHERE status = 'new'" ) ];
	}
...
function site_pulse_ajax_get_action_items(): void {
	...
	// Pull the full in-scope set (all non-pending statuses) once, so the label list reflects EVERY label
	// the user has — open or completed — regardless of the current status/label filter.
	$all = site_pulse_get_action_items( $args );

	// Distinct labels for the filter dropdown (case-insensitive, natural sort).
	$cat_keys = [];
	foreach ( $all as $r ) { $c = trim( (string) $r['category'] ); if ( '' !== $c ) $cat_keys[ $c ] = true; }
	$categories = array_keys( $cat_keys );
	natcasesort( $categories );
	$categories = array_values( $categories );

	// Apply the status + label filters in PHP against that one fetch.
	$status   = ! empty( $_POST['status'] ) ? sanitize_text_field( $_POST['status'] ) : '';
	$category = isset( $_POST['category'] ) ? sanitize_text_field( wp_unslash( $_POST['category'] ) ) : '';
	$items = array_values( array_filter( $all, function ( $r ) use ( $status, $category ) {
...
function site_pulse_ajax_add_action_note(): void {
	...
	$notes[] = current_time( 'n/j' ) . ' — ' . $note; // stamp each added note with a short M/D date
	$meta['notes'] = $notes;
	...
	// Joint item (assigned to several people): keep EVERY copy in the shared group in sync — the
	// rewritten description, due date, and notes — so adding details shows for all parties, not just
	// the person who added them (mirrors how completing a grouped item closes everyone's copy).
...
function site_pulse_ajax_set_action_due_date(): void {
	...
	$now = current_time( 'mysql' );
	$wpdb->update( site_pulse_table( 'action_items' ), [ 'due_date' => $due, 'updated_at' => $now ], [ 'id' => $item_id ] );

	// Joint item: move the due date on every copy in the shared group, so it stays in sync.
	$group = (string) ( $item['group_id'] ?? '' );
	if ( '' !== $group ) {
		$wpdb->update( site_pulse_table( 'action_items' ), [ 'due_date' => $due, 'updated_at' => $now ], [ 'group_id' => $group ] );
	}
...
function site_pulse_ajax_complete_action_item(): void {
	...
	// Tell everyone INVOLVED it's done — every person who held a copy (a shared group) PLUS the item's
	// creator — except whoever just completed it. So on a 2+ person item everyone gets the "completed"
	// ping no matter who closes it; on a solo item it's just the creator (when they didn't close it).
	$recipients = [];
	if ( '' !== $group ) {
		$recipients = array_map( 'intval', $wpdb->get_col( $wpdb->prepare(
			"SELECT DISTINCT user_id FROM " . site_pulse_table( 'action_items' ) . " WHERE group_id = %s",
			$group
		) ) );
	} else {
		$recipients[] = (int) $item['user_id'];
	}
	$creator = (int) ( $item['created_by'] ?? 0 );
	if ( $creator ) $recipients[] = $creator;
	$recipients = array_values( array_unique( array_filter( $recipients, fn( $u ) => $u && $u !== $me ) ) );

	if ( $recipients ) {
		$who = site_pulse_display_name( $me );
		$msg = sprintf( '%s completed an action item: %s', $who, $item['description'] );
		foreach ( $recipients as $uid ) {
			site_pulse_notify( $uid, 'action_item', $msg, $item_id, 'action_item' );
			if ( function_exists( 'site_pulse_push_send' ) ) site_pulse_push_send( $uid ); // bell + wake devices
		}
	}
...
// Edit an item's original text + label — owner only.
add_action( 'wp_ajax_site_pulse_update_action_item', 'site_pulse_ajax_update_action_item' );
function site_pulse_ajax_update_action_item(): void {
	check_ajax_referer( 'site_pulse_nonce', 'nonce' );
	$me      = site_pulse_effective_user_id();
	$item_id = (int) ( $_POST['item_id'] ?? 0 );
	$desc    = sanitize_text_field( wp_unslash( $_POST['description'] ?? '' ) );
...
// Reopen a completed item (undo an accidental "Mark Complete") — owner only. Clears the resolution and
// puts it back on the open list; reopens the whole shared group if it was a group item.
add_action( 'wp_ajax_site_pulse_reopen_action_item', 'site_pulse_ajax_reopen_action_item' );
function site_pulse_ajax_reopen_action_item(): void {
	check_ajax_referer( 'site_pulse_nonce', 'nonce' );
	$me      = site_pulse_effective_user_id();
	$item_id = (int) ( $_POST['item_id'] ?? 0 );
	if ( ! $item_id ) wp_send_json_error( [ 'message' => 'Invalid item.' ] );
	...
	$item = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM " . site_pulse_table( 'action_items' ) . " WHERE id = %d", $item_id ), ARRAY_A );
	if ( ! $item ) wp_send_json_error( [ 'message' => 'Item not found.' ] );
	if ( (int) $item['user_id'] !== $me ) wp_send_json_error( [ 'message' => 'Only the owner can reopen this item.' ] );
	...
	if ( '' !== $group ) {
		$wpdb->query( $wpdb->prepare(
			"UPDATE " . site_pulse_table( 'action_items' ) . "
			 SET status = 'open', resolved_at = NULL, resolved_by = NULL, resolution_note = NULL, updated_at = %s
			 WHERE group_id = %s AND status = 'resolved'",
			$now, $group
		) );
	}

	site_pulse_log( 'action_item_reopened', 'Action item reopened', [ 'item_id' => $item_id ] );
	wp_send_json_success();
}
...
function site_pulse_ajax_review_action_item(): void {
	...
	if ( (int) $item['user_id'] !== $user_id && ! site_pulse_god_can_override() ) {
...
function site_pulse_ajax_admin_save_setting(): void {
	...
	$notif_keys     = [ 'push_enabled', 'messages_email_enabled', 'notifications_email_enabled', 'push_contact_email' ];
...
/*--------------------------------------------------------------
# OKLCH color engine — perceptual, luminance-locked brand ramps
#
# The scheme is built deterministically from the company colors (no AI): the
# provided colors are sorted and assigned to three roles by their perceptual
# properties, then each role gets a 5-step ramp whose LIGHTNESS is locked to
# fixed targets — only the hue/chroma comes from the brand color. That makes
# the role tokens reliable no matter what colors are entered:
#   --sp-primary-*   = the DARK family   (primary-color is always dark → safe as a dark surface / dark text; -light/-lightest are guaranteed lighter)
#   --sp-secondary-* = the LIGHT family  (secondary-color is always light → safe as a light background; -dark/-darkest are guaranteed dark foregrounds)
#   --sp-accent-*    = the VIVID family  (kept saturated → buttons, active states, links, highlights)
# Math is OKLab/OKLCH (Björn Ottosson) so lightening/darkening stays on-hue
# instead of going chalky or muddy the way naive RGB white/black mixing does.
--------------------------------------------------------------*/

/** sRGB 0–255 channel → linear-light 0–1. */
function site_pulse_srgb_to_linear( float $c ): float {
	$c = $c / 255;
	return $c <= 0.04045 ? $c / 12.92 : pow( ( $c + 0.055 ) / 1.055, 2.4 );
}
/** Linear-light 0–1 → sRGB 0–255 (gamut-clamped). */
function site_pulse_linear_to_srgb( float $c ): float {
	$c = max( 0.0, min( 1.0, $c ) );
	$c = $c <= 0.0031308 ? 12.92 * $c : 1.055 * pow( $c, 1 / 2.4 ) - 0.055;
	return $c * 255;
}

/** Hex → [ L, C, H ] in OKLCH (L 0–1, C ~0–0.4, H radians). */
function site_pulse_hex_to_oklch( string $hex ): array {
	[ $r, $g, $b ] = site_pulse_hex_to_rgb( $hex );
	$r = site_pulse_srgb_to_linear( $r );
	$g = site_pulse_srgb_to_linear( $g );
	$b = site_pulse_srgb_to_linear( $b );

	$l = 0.4122214708 * $r + 0.5363325363 * $g + 0.0514459929 * $b;
	$m = 0.2119034982 * $r + 0.6806995451 * $g + 0.1073969566 * $b;
	$s = 0.0883024619 * $r + 0.2817188376 * $g + 0.6299787005 * $b;
	// inputs are non-negative, so a plain cube-root is safe.
...
/** [ L, C, H ] OKLCH → hex (gamut-clamped in linear light). */
function site_pulse_oklch_to_hex( float $L, float $C, float $H ): string {
	$A = $C * cos( $H );
	$B = $C * sin( $H );
	...
	$l = $l_ ** 3; $m = $m_ ** 3; $s = $s_ ** 3; // integer exponent: sign-safe

	$r =  4.0767416621 * $l - 3.3077115913 * $m + 0.2309699292 * $s;
	$g = -1.2684380046 * $l + 2.6097574011 * $m - 0.3413193965 * $s;
	$b = -0.0041960863 * $l - 0.7034186147 * $m + 1.7076147010 * $s;

	return site_pulse_rgb_to_hex(
		site_pulse_linear_to_srgb( $r ),
		site_pulse_linear_to_srgb( $g ),
		site_pulse_linear_to_srgb( $b )
	);
}

/**
 * Build one role ramp: the lightness of every rung is FIXED (so the role token
 * is predictable), the hue is the brand color's, the chroma is the brand color's
 * (capped, optionally floored to guarantee vividness) scaled per-rung so the
 * pale/deep ends don't go neon. $rungs = [ name => [ L_target, chroma_factor ] ].
 */
function site_pulse_oklch_ramp( string $srcHex, array $rungs, float $chromaCap, float $chromaFloor = 0.0 ): array {
	[ , $C0, $H ] = site_pulse_hex_to_oklch( $srcHex );
	$C0 = min( $C0, $chromaCap );
	$C0 = max( $C0, $chromaFloor );
	$out = [];
	foreach ( $rungs as $name => $spec ) {
		[ $L, $cf ] = $spec;
		$out[ $name ] = site_pulse_oklch_to_hex( $L, $C0 * $cf, $H );
	}
	return $out;
}

/**
 * Assign the provided company colors to the three roles, by ENTRY ORDER:
 *   - the 3rd color entered is the ACCENT (people pick their tertiary/accent color last);
 *   - of the first two, the darker is primary and the lighter is secondary.
 * With 2 colors there's no accent slot, so darker=primary, lighter=secondary, accent=more saturated.
 * With 1, all three reuse it. Order is preserved (no dedupe) so the slots stay meaningful.
 * Returns [ 'primary'=>hex, 'secondary'=>hex, 'accent'=>hex ] or [] if none.
 */
function site_pulse_assign_brand_roles( array $colors ): array {
	$cols = [];
	foreach ( $colors as $c ) {
		$h = site_pulse_sanitize_hex( (string) $c );
		if ( $h !== '' ) $cols[] = $h;   // keep entry order — slot 3 is the accent
	}
	...
	$n = count( $cols );
	if ( $n === 1 ) {
		return [ 'primary' => $cols[0], 'secondary' => $cols[0], 'accent' => $cols[0] ];
	}

	if ( $n === 2 ) {
		$La = site_pulse_hex_to_oklch( $cols[0] )[0]; $Lb = site_pulse_hex_to_oklch( $cols[1] )[0];
		$Ca = site_pulse_hex_to_oklch( $cols[0] )[1]; $Cb = site_pulse_hex_to_oklch( $cols[1] )[1];
		return [
			'primary'   => $La <= $Lb ? $cols[0] : $cols[1],
			'secondary' => $La <= $Lb ? $cols[1] : $cols[0],
			'accent'    => $Ca >= $Cb ? $cols[0] : $cols[1],
		];
	}

	// 3 colors: slot 3 is the accent; the darker of the first two is primary, the lighter secondary.
	$L0 = site_pulse_hex_to_oklch( $cols[0] )[0];
	$L1 = site_pulse_hex_to_oklch( $cols[1] )[0];
	return [
		'primary'   => $L0 <= $L1 ? $cols[0] : $cols[1],
		'secondary' => $L0 <= $L1 ? $cols[1] : $cols[0],
		'accent'    => $cols[2],
	];
}

/**
 * The primary (DARK) ramp. primary-color tracks the brand color's OWN lightness so it's as dark as
 * what the admin picked — only capped to a ceiling so a medium pick still darkens enough to stay a
 * legible dark surface/text (and floored so pure black still has a little life). The light rungs are
 * fixed up high for tints/hover-fills; the deep rungs sit below the color.
 */
function site_pulse_oklch_primary_ramp( string $srcHex ): array {
	[ $L0, $C0, $H ] = site_pulse_hex_to_oklch( $srcHex );
	// primary-color = the picked color VERBATIM when it's already a usable dark tone — no lightening,
	// no desaturating (reducing chroma was making dark reds read as a washed, lighter maroon). Only a
	// genuinely light pick is darkened to OKLCH L 0.42 to serve as the dark UI tone. Deep rungs sit
	// below the color at full chroma; only the pale tints ease the chroma so they aren't neon.
	if ( $L0 <= 0.45 ) {
		$color = $srcHex; $Lc = $L0; $Cc = $C0;
	} else {
		$Lc = 0.42; $Cc = $C0; $color = site_pulse_oklch_to_hex( $Lc, $Cc, $H );
	}
	$tintC = min( $C0, 0.10 );
	return [
		'lightest' => site_pulse_oklch_to_hex( 0.965, $tintC * 0.45, $H ),
		'light'    => site_pulse_oklch_to_hex( 0.880, $tintC * 0.80, $H ),
		'color'    => $color,
		'dark'     => site_pulse_oklch_to_hex( max( 0.12, $Lc - 0.09 ), $Cc, $H ),
		'darkest'  => site_pulse_oklch_to_hex( max( 0.08, $Lc - 0.17 ), $Cc, $H ),
	];
}

/**
 * The secondary (LIGHT) ramp. secondary-color tracks the brand color's own lightness so it's as light
 * as what the admin picked — floored so a mid pick still lightens enough to be a usable light
 * background. Chroma is kept low (muted). The deep rungs are fixed low for dark foregrounds on it.
 */
function site_pulse_oklch_secondary_ramp( string $srcHex ): array {
	[ $L0, $C0, $H ] = site_pulse_hex_to_oklch( $srcHex );
	// secondary-color = the picked color VERBATIM when it's already light enough to be a background;
	// only a darker pick is lightened to OKLCH L 0.88 to serve as the light surface. Deep rungs are
	// fixed low for dark foregrounds on it. Chroma kept muted on the synthesized tones.
	if ( $L0 >= 0.82 ) {
		$color = $srcHex; $Lc = $L0; $Cc = $C0;
	} else {
		$Lc = 0.88; $Cc = min( $C0, 0.05 ); $color = site_pulse_oklch_to_hex( $Lc, $Cc, $H );
	}
	return [
		'lightest' => site_pulse_oklch_to_hex( min( 0.995, $Lc + 0.05 ), min( $C0, 0.04 ) * 0.6, $H ),
		'light'    => site_pulse_oklch_to_hex( min( 0.985, $Lc + 0.025 ), min( $C0, 0.05 ) * 0.8, $H ),
		'color'    => $color,
		'dark'     => site_pulse_oklch_to_hex( 0.45, min( $C0, 0.06 ), $H ),
		'darkest'  => site_pulse_oklch_to_hex( 0.30, min( $C0, 0.06 ), $H ),
	];
}

/**
 * The accent (vivid) ramp keeps the brand pop's OWN lightness for accent-color (clamped to a vivid
 * band) so buttons stay on-brand; the other rungs are offset around it. Chroma is floored so the
 * accent stays lively even from a muted input. (Unlike primary/secondary, the accent's lightness is
 * NOT locked — accent is used as a fill with computed-contrast text, so its lightness can be free.)
 */
function site_pulse_oklch_accent_ramp( string $srcHex ): array {
	...
	return [
		...
		'light'    => site_pulse_oklch_to_hex( min( 0.88, $Lc + 0.16 ), $C0 * 0.70, $H ),
		'color'    => site_pulse_oklch_to_hex( $Lc, $C0, $H ),
		'dark'     => site_pulse_oklch_to_hex( max( 0.34, $Lc - 0.14 ), $C0, $H ),
		'darkest'  => site_pulse_oklch_to_hex( max( 0.26, $Lc - 0.26 ), $C0 * 0.95, $H ),
	];
}

/**
 * The complete CSS-variable map for a scheme built from the company colors.
 * Emits the three locked-lightness families (+ each one's best-contrast text),
 * lightly-branded neutrals, and fixed semantic colors, then runs the WCAG
 * contrast guard. Deterministic — same colors in, same scheme out.
 */
function site_pulse_build_color_scheme( array $colors ): array {
	$roles = site_pulse_assign_brand_roles( $colors );
	if ( ! $roles ) return [];

	// [ rung => [ locked OKLCH lightness, chroma scale ] ]
	$ramps = [
		'primary'   => site_pulse_oklch_primary_ramp( $roles['primary'] ),
		'secondary' => site_pulse_oklch_secondary_ramp( $roles['secondary'] ),
		'accent'    => site_pulse_oklch_accent_ramp( $roles['accent'] ),
	];

	$vars = [];
	foreach ( $ramps as $fam => $ramp ) {
		$vars[ "--sp-{$fam}-lightest" ] = $ramp['lightest'];
		$vars[ "--sp-{$fam}-light" ]    = $ramp['light'];
		$vars[ "--sp-{$fam}-color" ]    = $ramp['color'];
		$vars[ "--sp-{$fam}-dark" ]     = $ramp['dark'];
		$vars[ "--sp-{$fam}-darkest" ]  = $ramp['darkest'];
		$vars[ "--sp-{$fam}-contrast" ] = site_pulse_best_foreground( [ $ramp['color'] ] );
	}
	$vars['--sp-accent-hover'] = $ramps['accent']['dark'];
	$vars['--sp-accent-glow']  = site_pulse_color_rgba( $ramps['accent']['color'], 0.18 ); // active-item edge glow
	$vars['--sp-primary-line'] = 'rgba(255,255,255,0.10)';                                 // hairline on the dark sidebar

	// Lightly-branded neutrals (tinted by the brand hues, kept near-neutral by tiny chroma).
	$pH = site_pulse_hex_to_oklch( $roles['primary'] )[2];
	$sH = site_pulse_hex_to_oklch( $roles['secondary'] )[2];
	$vars['--sp-text']           = site_pulse_oklch_to_hex( 0.27, 0.012, $pH );
	$vars['--sp-text-secondary'] = site_pulse_oklch_to_hex( 0.50, 0.012, $pH );
	$vars['--sp-text-light']     = site_pulse_oklch_to_hex( 0.66, 0.012, $pH );
	$vars['--sp-bg']             = site_pulse_oklch_to_hex( 0.975, 0.010, $sH );
	$vars['--sp-bg-white']       = '#ffffff';
	$vars['--sp-border']         = site_pulse_oklch_to_hex( 0.900, 0.012, $sH );
	$vars['--sp-border-focus']   = $ramps['accent']['light'];

	// Fixed semantic colors — meaning beats brand-matching; keep green/amber/red recognizable.
	$vars['--sp-success']       = '#2f8f57';
	$vars['--sp-success-light'] = site_pulse_color_mix_white( '#2f8f57', 0.90 );
	$vars['--sp-warning']       = '#c77d18';
...
/**
 * AI-tuned scheme: Claude evaluates the company colors and returns the three families + neutrals +
 * semantics, which we lay over the deterministic build (so any field the model omits still has a
 * sane value), derive the dependent tokens, and run the WCAG guard. Returns
 * [ 'vars'=>map, 'rationale'=>string ] or NULL if AI is unavailable/failed (caller falls back to the
 * deterministic build). Same token model as build_color_scheme() — the CSS is unchanged.
 */
function site_pulse_ai_build_color_scheme( array $brand, &$debug = null ): ?array {
	$base = site_pulse_build_color_scheme( $brand );
	if ( ! $base ) { $debug = 'no-colors'; return null; }
	if ( ! site_pulse_get_api_key() ) { $debug = 'no-api-key'; return null; }

	$roles   = site_pulse_assign_brand_roles( $brand );   // slot 3 = accent; darker of 1&2 = primary
	$assign  = "- primary family base (the dark UI tone): {$roles['primary']}\n";
	$assign .= "- secondary family base (the light backgrounds): {$roles['secondary']}\n";
	$assign .= "- accent family base (the pop): {$roles['accent']}\n";

	$shape = '{'
		. '"primary":{"lightest":"#rrggbb","light":"#rrggbb","color":"#rrggbb","dark":"#rrggbb","darkest":"#rrggbb"},'
		. '"secondary":{"lightest":"#rrggbb","light":"#rrggbb","color":"#rrggbb","dark":"#rrggbb","darkest":"#rrggbb"},'
		. '"accent":{"lightest":"#rrggbb","light":"#rrggbb","color":"#rrggbb","dark":"#rrggbb","darkest":"#rrggbb"},'
		. '"neutral":{"text":"#rrggbb","text_secondary":"#rrggbb","text_light":"#rrggbb","bg":"#rrggbb","border":"#rrggbb"},'
		. '"semantic":{"success":"#rrggbb","warning":"#rrggbb","danger":"#rrggbb"},'
		. '"rationale":"one short sentence"}';

	$system = 'You are a senior brand and UI designer building an accessible color scheme for a web-app dashboard. Reply with ONLY one JSON object — no prose, no markdown fences.';

	$prompt  = "Build a complete, WCAG-AA color scheme for an internal web-app dashboard. The role of each company color is ALREADY decided — use these exact base colors, do NOT reassign or swap them:\n{$assign}\n";
	$prompt .= "For each family, use the assigned base color VERBATIM as its \"color\" tone (keep it rich and recognizable — do NOT wash it out, desaturate it, or shift its hue) and build the other four tones around it:\n";
	$prompt .= "- \"primary\" = the DARK family: navigation sidebar background, headings, dark text. Build lighter tints above the base and deeper shades below it. White text must read on primary.color — if the assigned base is too light for that, deepen it just enough.\n";
	$prompt .= "- \"secondary\" = the LIGHT family: light card/panel backgrounds. Dark text must read on secondary.color — if the assigned base is too dark, lighten it just enough.\n";
	$prompt .= "- \"accent\" = the VIVID family: buttons, links, the active menu item, highlights.\n";
	$prompt .= "Give each family 5 tones strictly ordered lightest > light > color > dark > darkest, each clearly distinct.\n";
	$prompt .= "Also: \"neutral\" — text (near-black body text), text_secondary, text_light, bg (very light page tint), border (subtle light divider); and \"semantic\" — success (green), warning (amber), danger (red).\n";
	$prompt .= "Every text/background pair must be clearly legible; favor light backgrounds with dark text. Use #rrggbb hex for every value.\n";
	$prompt .= "Return ONLY this JSON shape:\n{$shape}";

	$resp = site_pulse_call_claude( $prompt, $system, [ 'max_tokens' => 1200, 'timeout' => 45 ], $debug );
	if ( $resp === null ) return null;

	$json = trim( $resp );
	if ( preg_match( '/\{.*\}/s', $json, $m ) ) $json = $m[0];
	$p = json_decode( $json, true );
	if ( ! is_array( $p ) ) { $debug = 'unparseable AI response'; return null; }

	$vars = $base;
	$applyFam = function ( $key, $obj ) use ( &$vars ) {
		if ( ! is_array( $obj ) ) return;
		foreach ( [ 'lightest', 'light', 'color', 'dark', 'darkest' ] as $t ) {
			$h = site_pulse_sanitize_hex( (string) ( $obj[ $t ] ?? '' ) );
			if ( $h !== '' ) $vars[ "--sp-{$key}-{$t}" ] = $h;
		}
	};
	$applyFam( 'primary',   $p['primary']   ?? null );
	$applyFam( 'secondary', $p['secondary'] ?? null );
	$applyFam( 'accent',    $p['accent']    ?? null );

	if ( is_array( $p['neutral'] ?? null ) ) {
		$nmap = [ 'text' => '--sp-text', 'text_secondary' => '--sp-text-secondary', 'text_light' => '--sp-text-light', 'bg' => '--sp-bg', 'border' => '--sp-border' ];
		foreach ( $nmap as $k => $var ) {
			$h = site_pulse_sanitize_hex( (string) ( $p['neutral'][ $k ] ?? '' ) );
			if ( $h !== '' ) $vars[ $var ] = $h;
		}
	}
	if ( is_array( $p['semantic'] ?? null ) ) {
		foreach ( [ 'success', 'warning', 'danger' ] as $k ) {
			$h = site_pulse_sanitize_hex( (string) ( $p['semantic'][ $k ] ?? '' ) );
			if ( $h !== '' ) { $vars[ "--sp-{$k}" ] = $h; $vars[ "--sp-{$k}-light" ] = site_pulse_color_mix_white( $h, 0.90 ); }
		}
	}

	// Dependent tokens derived from the (possibly AI-chosen) family colors.
	$vars['--sp-primary-contrast']   = site_pulse_best_foreground( [ $vars['--sp-primary-color'] ] );
	$vars['--sp-secondary-contrast'] = site_pulse_best_foreground( [ $vars['--sp-secondary-color'] ] );
	$vars['--sp-accent-contrast']    = site_pulse_best_foreground( [ $vars['--sp-accent-color'] ] );
	$vars['--sp-accent-hover']       = $vars['--sp-accent-dark'];
	$vars['--sp-accent-glow']        = site_pulse_color_rgba( $vars['--sp-accent-color'], 0.18 );
	$vars['--sp-border-focus']       = $vars['--sp-accent-light'];
	$vars['--sp-bg-white']           = '#ffffff';

	$rationale = isset( $p['rationale'] ) ? sanitize_text_field( (string) $p['rationale'] ) : '';
	return [ 'vars' => site_pulse_enforce_contrast( $vars ), 'rationale' => $rationale ];
}
...
function site_pulse_enforce_contrast( array $v ): array {
	...
	$rules = [
		'--sp-primary-contrast'   => [ [ '--sp-primary-color' ], 4.5 ],   // light text on the dark family
		'--sp-accent-contrast'    => [ [ '--sp-accent-color' ], 4.5 ],    // text on accent buttons/fills
		'--sp-secondary-contrast' => [ [ '--sp-secondary-color' ], 4.5 ], // dark text on the light family
...
function site_pulse_ajax_admin_save_color_scheme(): void {
	...
	// Persist BOTH the company colors (to repopulate the form) AND the previewed scheme map (the
	// AI-tuned result the admin just approved) — AI can't run on every render, so the chosen map is
	// what active_vars emits. Order preserved (slot 3 = accent).
	$brandRaw = $_POST['brand_colors'] ?? '';
	$blist    = is_string( $brandRaw ) ? json_decode( wp_unslash( $brandRaw ), true ) : ( is_array( $brandRaw ) ? $brandRaw : [] );
	if ( ! is_array( $blist ) ) $blist = [];
	$bclean = [];
	foreach ( $blist as $c ) { $h = site_pulse_sanitize_hex( (string) $c ); if ( $h !== '' ) $bclean[] = $h; }
	$bclean = array_slice( $bclean, 0, 3 );
	if ( ! $bclean ) wp_send_json_error( [ 'message' => 'Enter at least one company color.' ] );

	$raw  = $_POST['vars'] ?? '';
	$vars = is_string( $raw ) ? json_decode( wp_unslash( $raw ), true ) : ( is_array( $raw ) ? $raw : [] );
	$allowed = array_keys( site_pulse_build_color_scheme( [ '#888888' ] ) ); // canonical token set
	$clean = [];
	if ( is_array( $vars ) ) {
		foreach ( $vars as $k => $v ) {
	...
	if ( ! $clean ) $clean = site_pulse_build_color_scheme( $bclean ); // nothing usable sent → fresh build

	site_pulse_set_setting( 'color_brand', wp_json_encode( $bclean ) );
	site_pulse_set_setting( 'color_vars', wp_json_encode( $clean ) );
	global $wpdb;
	$wpdb->delete( site_pulse_table( 'config' ), [ 'config_key' => 'color_mood' ] );
...
/**
 * Scheme generator (live preview, NOT saved until Save): Claude evaluates the company colors and
 * returns the three-family scheme. If AI is unavailable (no key / failure) it falls back to the
 * deterministic OKLCH build so the button always works. Same token model either way.
 */
add_action( 'wp_ajax_site_pulse_admin_generate_color_scheme', 'site_pulse_ajax_admin_generate_color_scheme' );
function site_pulse_ajax_admin_generate_color_scheme(): void {
	check_ajax_referer( 'site_pulse_nonce', 'nonce' );
	if ( ! site_pulse_admin_check( 'manage_settings' ) ) return;
	...
	if ( is_array( $list ) ) {
		foreach ( $list as $c ) {
			...
			if ( $hex !== '' ) $brand[] = $hex;   // keep entry order — slot 3 is the accent
		}
	}
	$brand = array_slice( $brand, 0, 3 );
	if ( ! $brand ) wp_send_json_error( [ 'message' => 'Enter at least one company color.' ] );

	$debug = null;
	$ai    = site_pulse_ai_build_color_scheme( $brand, $debug );
	if ( $ai ) {
		wp_send_json_success( [ 'vars' => $ai['vars'], 'rationale' => ( $ai['rationale'] ?: 'AI-tuned from your colors.' ) ] );
	}

	// Fallback: deterministic build so the scheme still works without a key / on AI failure.
	$note = ( $debug === 'no-api-key' )
		? 'Built locally — add a Claude API key under Settings → API Keys for AI tuning.'
		: 'Built locally (AI unavailable' . ( $debug ? ": $debug" : '' ) . ').';
	wp_send_json_success( [ 'vars' => site_pulse_build_color_scheme( $brand ), 'rationale' => $note ] );
}
...
function site_pulse_maybe_fix_legacy_slashes(): void {
	...
	if ( ! is_user_logged_in() || ! site_pulse_god_can_override() ) wp_die( 'Not authorized.' );
	nocache_headers();
...
/**
 * How tolls get onto a reimbursement: 'tollguru' (auto-price each tolled leg via the TollGuru API —
 * the default) or 'ntta_csv' (admin uploads NTTA toll-statement CSVs and reconciles them against
 * routes). Kept as a setting so TollGuru can stay dormant for clients that prefer CSV reconciliation
 * without ripping the integration out.
 */
function site_pulse_toll_mode(): string {
	$m = site_pulse_get_setting( 'toll_mode', 'tollguru' );
	return $m === 'ntta_csv' ? 'ntta_csv' : 'tollguru';
}
	...
	if ( $raw !== '' ) {
		...
		if ( is_array( $decoded ) ) {
			foreach ( $decoded as $c ) {
				...
				$kids[ $ckey ] = $children;
				$vis[ $ckey ]  = ! array_key_exists( 'visible', $c ) || ! empty( $c['visible'] ); // default ON
			}
		}
	}
	$tree = [];
	foreach ( $flat as $key => $label ) {
		$tree[ $key ] = [ 'label' => $label, 'children' => $kids[ $key ] ?? [], 'visible' => $vis[ $key ] ?? true ];
	}
	return $tree;
}
...
function site_pulse_form_row( int $id, int $user_id ): ?array {
	...
	if ( ! $row ) return null;
	$is_god     = site_pulse_god_can_override();
...
function site_pulse_ajax_get_forms(): void {
	...
	// Hidden repositories are invisible to staff (those without manage_settings); admins/god still see
	// them. This applies to direct category access AND the cross-repository "All" view.
	$sees_hidden = $is_god || site_pulse_user_can( $user_id, 'manage_settings' );
	$hidden_keys = [];
	if ( ! $sees_hidden ) {
		foreach ( site_pulse_form_category_tree() as $hk => $hc ) if ( empty( $hc['visible'] ) ) $hidden_keys[] = $hk;
	}
	if ( ! $is_all && in_array( $category, $hidden_keys, true ) ) wp_send_json_error( [ 'message' => 'Unknown repository.' ] );

	global $wpdb;
	$tbl = site_pulse_table('forms');
	if ( $is_all ) {
		if ( $hidden_keys ) {
			$ph   = implode( ',', array_fill( 0, count( $hidden_keys ), '%s' ) );
			$rows = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $tbl WHERE category NOT IN ($ph) ORDER BY created_at DESC", ...$hidden_keys ), ARRAY_A ) ?: [];
		} else {
			$rows = $wpdb->get_results( "SELECT * FROM $tbl ORDER BY created_at DESC", ARRAY_A ) ?: [];
		}
	} else {
		$rows = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $tbl WHERE category = %s ORDER BY created_at DESC", $category ), ARRAY_A ) ?: [];
	}
	$can_upload = $is_god || site_pulse_user_can( $user_id, 'upload_forms' );
...
function site_pulse_ajax_upload_form(): void {
	check_ajax_referer( 'site_pulse_nonce', 'nonce' );
	$user_id = site_pulse_effective_user_id();
	if ( ! $user_id || ! ( site_pulse_god_can_override() || site_pulse_user_can( $user_id, 'upload_forms' ) ) ) wp_send_json_error( [ 'message' => 'You do not have permission to upload forms.' ] );
...
function site_pulse_ajax_get_form_settings(): void {
	...
	foreach ( site_pulse_form_category_tree() as $key => $cat ) {
		...
		$cats[] = [ 'key' => $key, 'label' => $cat['label'], 'count' => $counts[ $key ] ?? 0, 'children' => $kids, 'visible' => ! empty( $cat['visible'] ) ];
	}
...
function site_pulse_ajax_save_form_categories(): void {
	...
	foreach ( $raw as $c ) {
		...
		$final[ $key ]   = $label;
		$visible[ $key ] = ! array_key_exists( 'visible', $c ) || ! empty( $c['visible'] ); // default ON
	...
	foreach ( $final as $k => $l ) {
		...
		$store[] = [ 'key' => $k, 'label' => $l, 'visible' => ! empty( $visible[ $k ] ), 'children' => $kids ];
	}
...
function site_pulse_ajax_delete_mileage_entry(): void {
	...
	if ( ! $entry ) wp_send_json_error( [ 'message' => 'Entry not found.' ] );
	if ( (int) $entry['user_id'] !== $user_id && ! site_pulse_god_can_override() ) {
		wp_send_json_error( [ 'message' => 'Not authorized.' ] );
	}
...
function site_pulse_ajax_admin_save_toll_settings(): void {
	...
	if ( isset( $_POST['mode'] ) ) {
		site_pulse_set_setting( 'toll_mode', $_POST['mode'] === 'ntta_csv' ? 'ntta_csv' : 'tollguru' );
	}
	wp_send_json_success( [
		'mode'         => site_pulse_toll_mode(),
		'vehicle_type' => site_pulse_toll_vehicle_type(),
