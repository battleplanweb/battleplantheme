...
function bp_render_form($atts, $content = null) {
	...
	$out .= '<form class="bp-form-el init" novalidate enctype="multipart/form-data" data-form-id="' . esc_attr($form_id) . '">';
...
function bp_handle_form_submission(WP_REST_Request $request) {
	...
	$default_to    = sanitize_email($params['bp_recipient'] ?? ($customer_info['email'] ?? get_option('admin_email')));
...
function bp_resolve_recipient($params, $fields, $default_to) {
	...
	$valid = [];
	foreach (explode(',', (string)$map[$picked]) as $raw) {
		$clean = sanitize_email(trim($raw));
		if ($clean) $valid[] = $clean;
	}
	if (empty($valid)) {
		return ['email' => $default_to, 'label' => '', 'tampered' => true];
	}

	return ['email' => implode(', ', $valid), 'label' => $picked, 'tampered' => false];
}
...
function bp_collect_submitted_fields($params) {
	$out = [];
	$skip = ['bp_form', 'bp_t', 'bp_h', 'bp_hp', 'bp_subject', 'bp_recipient', 'bp_redirect', 'bp_recipient_map', 'bp_recipient_hmac', 'bp_recipient_field', 'bp_accept_types', 'bp_accept_hmac', 'bp_max_mb', 'bp_max_mb_hmac', 'rest_route'];
