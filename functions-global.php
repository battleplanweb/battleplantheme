...
if ( !defined('_BP_VERSION') ) define( '_BP_VERSION', '23.8.1' );
...
if ( !is_admin() && !isset($_COOKIE['user-display-loc']) ) :
	...
	if ( $location == "none" ) :
		if ( ( !get_option('jobsite_geo') || get_option('jobsite_geo')['install'] != 'true' ) && ( preg_match('/-[a-z]{2}$/', $page_slug) === 1 && !in_array( substr($page_slug, -2), $common)) ) :
			$pieces = explode(' ', ucwords(str_replace('-', ' ', $page_slug))); 
			$state = array_pop($pieces);
			$city = implode(' ', $pieces);
			$location = $city.', '.$state;
		elseif ( isset($_COOKIE['user-city']) && in_array($_COOKIE['user-city'], $GLOBALS['customer_info']['service-areas']) ) : $location = $_COOKIE['user-city'].', '.$_COOKIE['user-state']; 	
		else:
			$location = $GLOBALS['customer_info']['default-loc'];
		endif;
	endif;

	$_COOKIE['user-display-loc'] = $location;
endif;

if ( !defined('_USER_DISPLAY_LOC') ) define( '_USER_DISPLAY_LOC', $_COOKIE['user-display-loc'] );
