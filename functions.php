...
if ( ! function_exists( 'battleplan_setup' ) ) :
	...
	function battleplan_setup() {
		 ...
		 * If you're building a theme based on Battle Plan Web Design, use a find and replace
		 * to change 'battleplan' to the name of your theme in all the template files.
		 */
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'battleplan' ),
			)
		);
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'battleplan_custom_background_args',
				array(
...
function battleplan_scripts() {
	wp_enqueue_style( 'battleplan-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'battleplan-style', 'rtl', 'replace' );

	wp_enqueue_script( 'battleplan-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
