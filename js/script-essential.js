...
// Handle locking divs to top of screen on scroll	
	...
	const checkFixedEls = [ '#page > #desktop-navigation', '.top-strip', '#masthead' ];

	const fixedElement = checkFixedEls.map(selector => getObject(selector))
		.find(el => el && getComputedStyle(el).position === 'fixed');

	if (fixedElement) {
		const targetElement = fixedElement.matches('#desktop-navigation') ? getObject('#masthead') : fixedElement.nextElementSibling;
		if (targetElement) {
